...
export class FavoritosComponent implements OnInit {
  ...
  getLibrosIds(): void {
    this.http.get<any[]>(`https://wattbook-5pt2.onrender.com/libros-favoritos/yourFaves/${this.userId}`, { observe: 'response' })
      .subscribe({
        next: response => {
  ...
  getAutoresIds(): void {
    this.http.get<any[]>(`https://wattbook-5pt2.onrender.com/seguidores/tusSeguidos/${this.userId}`, { observe: 'response' })
      .subscribe({
        next: response => {
  ...
  getLibrosFavoritos(): void {
    this.librosIds.forEach(id => {
      this.http.get<any>(`https://wattbook-5pt2.onrender.com/libros/${id}`)
        .subscribe({
          next: libro => {
            ...
            this.http.get<any>(`https://wattbook-5pt2.onrender.com/usuario/${autorId}/perfil`)
              .subscribe({
                next: autor => {
